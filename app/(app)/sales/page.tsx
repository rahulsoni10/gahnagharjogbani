...
import { formatCurrency, calcPureWeight, goldPurityLabel } from "@/lib/calculations";
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">All recorded sales. Undo a sale to return the item to stock.</p>
        </div>
      ...
      {/* Summary strip */}
      {filtered.length > 0 && (
        <div className="flex gap-6 px-4 py-3 bg-muted/40 rounded-lg border text-sm">
          <div>
            <span className="text-muted-foreground">Sales shown: </span>
            <strong>{filtered.length}</strong>
          </div>
          <div>
            <span className="text-muted-foreground">Total revenue: </span>
            <strong className="text-emerald-700">{formatCurrency(totalRevenue)}</strong>
          </div>
        </div>
      )}
                  ...
                  <TableRow key={sale.id} className="hover:bg-muted/30">
                    <TableCell>
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground font-mono">{sale.item.itemCode}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-col gap-1">
                        <Badge variant="outline" className="text-xs w-fit">{sale.item.type}</Badge>
                        <Badge
                          variant="outline"
                          className={`text-xs w-fit ${isGold ? "bg-amber-50 text-amber-800 border-amber-200" : "bg-slate-50 text-slate-700 border-slate-200"}`}
                        >
